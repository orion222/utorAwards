...
class PromotionService {
    static async createPromotion(name, description, type, startTime, endTime, minSpending, rate, points, utorid) {
        const creator = await prisma.user.findUnique({
            where: { utorid: utorid }
        });

        if (!creator) throw NotFoundError("Not Found: User not found to create promotion");

        return await prisma.promotion.create({
            data: {
                type: type === 'automatic' ? PromotionType.automatic : PromotionType.onetime,
                name: name,
    ...
    static async retrievePromotions(userId, role, name, type, page, limit, started, ended) {
        const where = {};
    ...
    static async retrieveSinglePromotion(role, promotionId) {
        ...
        if (role === RoleType.regular || role === RoleType.cashier) {
            ...
            if (promotion.startTime > currentTime || promotion.endTime < currentTime) {
                throw new NotFoundError(`Promotion with id ${promotionId} has already started or ended`);
            }
        }
    ...
    static async updateSinglePromotion(promotionId, name, description, type, startTime, endTime, minSpending, rate, points) {
        ...
        const currentTime = new Date();
        if ((name || description || type || startTime || minSpending || rate || points) && currentTime > promotion.startTime) {
            throw new ForbiddenError('Cannot update a promotion that has already started');
        }
        if (endTime && currentTime > promotion.endTime) {
            throw new ForbiddenError('Cannot update a promotion that has already ended');
        }
    ...
    static async deleteSinglePromotion(promotionId) {
        ...
        if (currentTime >= promotion.startTime) {
            throw new ForbiddenError('Cannot delete a promotion that has already started');
        }
