...
class EventService {
    static async createEvent(name, description, location, startTime, endTime, capacity, points, creatorId) {
    ...
    static async getFilteredEvents(name, location, started, ended, showFull, page = 1, limit = 10, published, role) {
        const filterDetails = {};
        if (name) {
    ...
    static async getSpecificEvent(eventId, userId, role) {
        ...
        const isAnOrganizer = (specificEvent.organizers).some(item => item.id === userId);
        
        if ((role === RoleType.regular || role === RoleType.cashier) && !isAnOrganizer) {
    ...
    static async updateEvent(id, updateParams) {
        ...
        const updatedEvent = await prisma.$transaction(async (tx) => {
            ...
            if ((updateParams.name || updateParams.description || updateParams.location || updateParams.capacity) && event.startTime < now) {
            ...
            if (updateParams.capacity && updateParams.capacity < currCapacity) {
                throw new BadRequestError("number of guests exceeds capacity reduction");
            }
            
            // check if change to points makes pointsAvailable negative
            if (updateParams.points && (updateParams.points - event.pointsAwarded < 0)) {
    ...
    static async deleteEvent(id) {
        const retVal = await prisma.$transaction(async (tx) => {
            ...
            if (event.published) throw new BadRequestError("event has already been published");
    ...
    static async rsvpForEvent(eventId, userId) {
        const signedUpUser = await prisma.$transaction(async (tx) => {
            ...
            const now = new Date();
            if (event.endTime < now) throw new BadRequestError("event has already ended");
            ...
            if (userAlreadyRSVP) throw new BadRequestError("user is already in guest list");
    ...
    static async addEventGuest(eventId, utorid){
        const response = await prisma.$transaction(async (tx) => {
            ...
            if (isOrganizer) throw new BadRequestError("User is an organizer of the event.");

            const now = Date.now()
            if (event.endTime < now || event.numGuests === event.capacity) throw new ExpiredError("Gone.")
            if (!event.published) throw new NotFoundError("Event is not visible to organizers yet");
