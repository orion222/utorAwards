...
class UserService {
    ...
    static async updateMyUserPassword(userId, oldPassword, newPassword) {
        ...
        const { password } = user;
        if (password && !(await bcrypt.compare(oldPassword, password))) throw new ForbiddenError("Incorrect old password");

        const passwordRegex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z0-9]).{8,20}$/;
        if (!passwordRegex.test(newPassword)) throw new BadRequestError("Invalid password format");
