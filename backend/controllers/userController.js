...
async function registerUser(req, res) {
    ...
    if ((utorid.length !== 7 && utorid.length !== 8) || name.length < 1 || name.length > 50 || !email.endsWith("@mail.utoronto.ca")) {
        return res.status(400).json({ error: 'Bad Request' });
    }
    ...
    try {
        ...
        res.status(409).json({ error: "User with given utorid/email already exists" });
    }
}
...
async function getFilteredUsers(req, res) {
    ...
    if (verified && verified !== 'true' && verified !== 'false') {
        return res.status(400).json({ error: 'Bad Request: invalid verified value' });
    }
    if (activated && activated !== 'true' && activated !== 'false') {
        return res.status(400).json({ error: 'Bad Request: invalid activated value' });
    }
    if (page && (!Number.isInteger(parseInt(page, 10)) || parseInt(page, 10) <= 0)) {
    ...
    if (pageNum < 1) {
        return res.status(400).json({ error: 'Bad Request: page must be at least 1' });
    }
    
    const filteredUsersData = await UserService.getFilteredUsers(name, role, verified, activated, pageNum, limitNum);
...
async function updateSpecificUser(req, res) {
  ...
  if (Object.keys(req.body).length === 0 || (!email && verified === null && suspicious === null && !role)) {
    return res.status(400).json({ error: 'Bad Request: at least one field is required' });
  }
  ...
  if (verified !== undefined && verified !== null) {
    if (typeof verified !== 'boolean') {
      return res.status(400).json({ error: 'Bad Request: verified must be a boolean' });
    }
    if (!verified) {
      return res.status(400).json({ error: 'Bad Request: cannot unverify a user' });
    }
  ...
  if (suspicious !== undefined && suspicious !== null) {
    if (typeof suspicious !== 'boolean') {
      return res.status(400).json({ error: 'Bad Request: suspicious must be a boolean' });
    }
    ...
    try {
      switch (role) {
        ...
        case 'cashier': {
          ...
          if ((suspicious !== undefined && suspicious !== null && suspicious) || userData.suspicious) {
            return res.status(403).json({ error: 'Forbidden: suspicious users cannot become cashiers' });
          }
...
async function createRedemption(req, res) {
    ...
    try {
        const newTransaction = await TransactionService.createRedemption(utorid, amount, remark ?? "");
        const response = {
            id: newTransaction.id,
            utorid: newTransaction.targetUser.utorid,
            type: type,
            processedBy: newTransaction.processedByUser ? newTransaction.processedByUser.utorid : null,
...
async function retrieveTransactions(req, res) {
    ...
    try {
        const transactionData = await TransactionService.retrieveTransactions(name, null, null, promotionId, type, relatedId, amount, operator, pageNum, limitNum);
...
async function createTransfer(req, res) {
    ...
    try {
        const { senderTransaction, recipientTransaction } = await TransactionService.createTransfer(senderId, recipientId, amountNum, remark);
        const response = {
...
async function updateMyUserInfo(req, res) {
    ...
    if (Object.keys(req.body).length === 0 || (!name && !email && !birthday && !avatar))
    ...
    try {
        const updatedUserInfo = await UserService.updateMyUserInfo(id, name, email, birthday, avatar);
...
async function updateMyPassword(req, res) {
    ...
    if (!old || !newPassword || typeof old !== "string" || typeof newPassword !== "string") {
        return res.status(400).json({ error: "Bad Request: Missing/invalid fields" });
    }
...
module.exports = { registerUser, getFilteredUsers, getSpecificUser, updateSpecificUser, createRedemption, createTransfer, retrieveTransactions, updateMyUserInfo, getMyUserInfo, updateMyPassword }
