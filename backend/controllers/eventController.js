...
async function getFilteredEvents(req, res) {
    ...
    if (showFull && showFull !== 'true' && showFull !== 'false') {
        return res.status(400).json({ error: 'Bad Request: invalid showFull value' });
    } 
    ...
    if (published && published !== 'true' && published !== 'false') {
        return res.status(400).json({ error: 'Bad Request: invalid published value' });
    } 
    if (published && (userRole !== RoleType.manager && userRole !== RoleType.superuser)) {
        return res.status(403).json({error: "only managers and superusers can see unpublished events"})
    }
    ...
    try {
        const filteredEventsData = await EventService.getFilteredEvents(name, location, started, ended, showFull, page, limit, published, userRole)
...
async function createEvent(req, res) {
    ...
    if (typeof name !== 'string' || typeof description !== 'string' || typeof location !== 'string' || typeof startTime !== 'string' || typeof endTime !== 'string' || !Number.isInteger(points)) {
        return res.status(400).json({ error: 'Bad Request' });
    }
    ...
    if (!isInISODateString(startTime) || !isInISODateString(endTime) || isNaN(start.getTime()) || isNaN(end.getTime()) || end < start || (Number.isInteger(capacity) && capacity <= 0) || points <= 0) {
        return res.status(400).json({ error: 'Bad Request' });
    }

    try {
        const cap = capacity === undefined ? null : capacity;
        const newEvent = await EventService.createEvent(name, description, location, start, end, cap, points, req.user.id);
...
async function createReward(req, res) {
    ...
    if (!type || type !== TransactionType.event) {
        return res.status(400).json({ error: `Bad Request: Invalid type given: ${type}, not 'event'` });
    }

    if (!Number.isInteger(amount) || amount <= 0) {
        return res.status(400).json({ error: `Bad Request: Invalid amount ${amount}` });
    }

    if (utorid && (typeof utorid !== "string" || (utorid.length !== 7 && utorid.length !== 8))) {
        return res.status(400).json({ error: `Bad Request: Invalid utorid ${utorid}` });
    }
    ...
    try {
        const newTransaction = await TransactionService.createEvent(creatorUtorid, utorid, type, amount, eventId);
...
async function getSpecificEvent(req, res) {
    ...
    const userRole = req.user.role;
    try {
        const eventData = await EventService.getSpecificEvent(eventId, userId, userRole);
...
module.exports = { createEvent, getFilteredEvents, createReward, getSpecificEvent, updateEvent, deleteEvent, signUpForEvent, removeFromEvent, addEventOrganizer, removeEventOrganizer, addEventGuest, removeEventGuest }
